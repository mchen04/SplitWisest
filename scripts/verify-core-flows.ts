...
function assertNoSecretText(value: unknown, label: string) {
  const text = JSON.stringify(value);
  for (const secret of ["password_hash", "recovery_codes", "sw_session"]) {
    assert(!text.includes(secret), `${label} leaked ${secret}`);
  }
}

async function cleanup() {
  const users = await sql`SELECT id FROM users WHERE username LIKE ${`core\\_%\\_${suffix}`} ESCAPE '\\'`;
...
async function request(path: string, opts: { cookie?: string; method?: string; body?: unknown; form?: FormData } = {}) {
...
async function signup(role: string) {
  const username = `core_${role}_${suffix}`;
  const displayName = `Core ${role} ${suffix}`;
...
async function login(username: string, rawPassword: string) {
